    ...
    <div style="width:300px; height:100vh; position: fixed; left:8%; z-index:-1;" class="d-flex align-items-center" id="moduless">
      <div style="background-color:green; width:100%; height:70%; opacity:0; display:none; transition:0.5s" id="modules" @mouseenter="moduleEnter()" @mouseleave="moduleLeave()">
        <ul class="d-flex flex-column justify-content-around h-100 text-left" style="list-style:none">
          <li v-if="role == 'trader'"><router-link style="color:white" to="/supplier/list">Suppliers</router-link></li>
          <li><router-link style="color:white" :to="role == 'trader' ? '/projects' : '/projects/owner/all'">Projects</router-link></li>
          <li><router-link style="color:white" :to="role == 'trader' ? '/produces' : '/produces/owner/all'">Produces</router-link></li>
          <li><router-link style="color:white" :to="role == 'trader' ? '/farms' : '/farms/owner/all'">{{ role == 'trader' ? 'Farms and Owners' : 'Farms' }}</router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" to="/supplyOrder/statusDashboard">Supply Purchase Orders</router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" to="/bid/orders">Bid Orders</router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" :to="`/messages/${getID}/trader`">Chat</router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" to="/supplyInventory/dashboard">Supply Inventory </router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" to="/supply/list"> Supply Profile </router-link></li>
          <li v-if="role == 'trader'"><router-link style="color:white" to="/reports/dashboard">Reports Dashboard</router-link></li>
          ...
          <li><router-link style="color:white" :to="role == 'farm_owner' ? `/reports/salesReport/owner`: `/reports/salesReport`">Sales Report</router-link></li>
          <li><router-link style="color:white" :to="role == 'trader' ? '/reports/ProfitSharing' : '/reports/profitSharing/owner'">Profit Sharing Report</router-link></li>
        </ul>
      </div>
    </div>

    ...
    <div class="container-fluid w-100 p-3 m-0" style="height:90%; background:#D8E9BE">
      <div class="w-100 h-100 d-flex" style="border:1px solid black">
        <div class="h-100" style="border-right:1px solid black; width:25%; overflow-y: scroll;">
          <h4 :class="selected_id != id ? 'ps-3 d-flex align-items-center distributor' : 'ps-3 d-flex align-items-center distributor selected'" style="height:7%;" @click="setSelectedId(id)" v-for="(id, index) in getFilteredNameList" :key="index">{{ getName(id) }}</h4>
        </div>
        <div class="h-100" style="width:75%;">
          <div id="chat" class="" style="width:100%; height:90%; overflow-y: scroll;">
            <div style="width:100%; clear: both;" :class="index != 0 ? 'mt-2 p-3' : 'mt-5 p-3'" v-for="(message, index) in getSelectedMsgs" :key="index">
              <div :ref="index == getSelectedMsgs.length - 1 ? `last${selected_id}` : ''" :class="message.message_sentBy == 'trader' ? 'p-3 text-center float-right sender' : 'p-3 text-center float-left recipient'" v-b-tooltip.hover :title="getDate(message)">{{ message.message_body }}</div>
            </div>                        
          </div>
...
export default {
    ...
    created(){
      ...
      .then(() => {        
        if(!this.selected_id){
          ...
          this.data.distributor_id = this.selected_id                             
        }                  
        this.readyApp()
        .then(() => {          
    ...
    methods: {
        ...
        getName(id){
          var distObj = this.getDistributors.filter((t) => {
        ...
        setSelectedId(id){
          ...
          this.selected_id = id         
          this.data.distributor_id = id   
          this.data.message_body = null      
        }
    },
    computed: {
      ...mapGetters(['getMessages', 'getDistributors']),
      getFilteredNameList(){
        var idss = new Set();
        if(this.getMessages){
          for(var msg of this.getMessages){
            idss.add(msg.distributor_id)
          } 
          var ids = [];
          for (var id of idss){
            ids.push(id)
      ...
      getSelectedMsgs(){
        ...
        if(this.getMessages){          
          var messages = this.getMessages.filter((msg) => {
            return parseInt(this.selected_id) === parseInt(msg.distributor_id)
          }) 
        }          
        return messages
      }

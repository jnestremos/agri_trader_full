  ...
  <div class="bidOrderDetails">
    ...
    <form class="ps-3 pe-4 w-100" @submit.prevent="sendOrder()">
        ...
        <div v-if="getOrder.bidOrder && getOrder.bidOrder.order_finalPrice" class="d-flex align-items-baseline justify-content-between w-100">
            <h5>Final Price: {{ getOrder.bidOrder.order_finalPrice.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) }}</h5>
        </div>
        ...
        <div v-if="bid_order_status_id != 1 || bid_order_status_id != 2 || bid_order_status_id != 3" :class="getOrder.bidOrder && getOrder.bidOrder.order_finalPrice ? 'd-flex align-items-baseline justify-content-between w-100 mt-2' : 'd-flex align-items-baseline justify-content-between w-100'">
            <h5 v-if="getOrder.bidOrder && getOrder.bidOrder.order_negotiatedPrice" class="me-3">Negotiated Price: {{ getOrder.bidOrder.order_negotiatedPrice.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) }}</h5>
        ...
        <div :class="bid_order_status_id != 1 || bid_order_status_id != 2 || bid_order_status_id != 3 ? 'd-flex align-items-baseline justify-content-between w-100 mt-2' : 'd-flex align-items-baseline justify-content-between w-100'">
            <h5 v-if="getOrder.contract">Trader's Price: {{ getOrder.bidOrder.order_traderPrice.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) }}</h5>
            <div v-if="bid_order_status_id != 3 && bid_order_status_id != 4 && bid_order_status_id != 5 && bid_order_status_id != 6 && bid_order_status_id != 7" class="d-flex align-items-baseline">
                ...
                <h5 v-else-if="getOrder.bidOrder && getOrder.bidOrder.order_negotiatedPrice" class="me-3">{{ getOrder.bidOrder.order_negotiatedPrice ?  getOrder.bidOrder.order_negotiatedPrice.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : '' }}</h5>
            </div>
        ...
        <div class="d-flex align-items-baseline justify-content-between w-100 mt-3">
            <h5 v-if="getOrder.bidOrder && getOrder.bidOrder.order_initialPrice">Asking Price: {{ getOrder.bidOrder.order_initialPrice ? getOrder.bidOrder.order_initialPrice.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : '' }}</h5>
            <div v-if="bid_order_status_id != 3 && bid_order_status_id != 4 && bid_order_status_id != 5 && bid_order_status_id != 6 && bid_order_status_id != 7" class="d-flex align-items-baseline">
                ...
                <h5 v-else-if="getOrder.bidOrder && getOrder.bidOrder.order_dpAmount" class="me-3">{{ getOrder.bidOrder.order_dpAmount ? getOrder.bidOrder.order_dpAmount.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : '' }}</h5>
            ...
            <h5 v-if="(bid_order_status_id == 4 && $route.query.finalPrice || bid_order_status_id == 4 && getOrder.on_hand_bid)">Total Amount: {{ getOrder.on_hand_bid ? getOrder.on_hand_bid.on_hand_bid_total.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : data.order_finalTotal }}</h5>
            <div v-if="bid_order_status_id == 7 && getOrder.refund && data.bid_order_acc_paymentMethod == 'Bank' && getOrder.bid_order_acc[0].bid_order_acc_type != 'Refund'" class="d-flex align-items-baseline">
        ...
        <div class="d-flex align-items-baseline justify-content-between w-100 mt-3">
            ...
            <div v-if="bid_order_status_id != 3 && bid_order_status_id != 4 && bid_order_status_id != 5 && bid_order_status_id != 6 && bid_order_status_id != 7" class="d-flex align-items-baseline">
                ...
                <h5 v-if="getOrder.bid_order_acc">{{ getDpDueDate }}</h5>
            </div>
            <h5 v-if="bid_order_status_id == 4 && data.order_finalPrice && data.order_finalQty && data.produce_trader_id && getDeliveryForm.bid_order && getDeliveryForm.bid_order.order_dpAmount || (bid_order_status_id == 4 && getOrder.on_hand_bid)">Paid Amount: {{ bid_order_status_id == 4 && getOrder.on_hand_bid ? getOrder.bidOrder.order_dpAmount.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) :getDeliveryForm.bid_order.order_dpAmount.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) }}</h5>
            <div v-if="bid_order_status_id == 7 && getOrder.refund && getOrder.bid_order_acc[0].bid_order_acc_type != 'Refund'" class="d-flex align-items-baseline">
        ...
        <div v-if="bid_order_status_id == 1 || bid_order_status_id == 2" class="d-flex align-items-baseline justify-content-between w-100 mt-3">
            <h5 v-if="getOrder.project_bid || (getOrder.on_hand_bid && getOrder.on_hand_bid.on_hand_bid_total)">Initial Total Balance: {{ getOrder.project_bid ? getOrder.project_bid.project_bid_total : getOrder.on_hand_bid.on_hand_bid_total.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) }}</h5>
        </div>
        <div class="d-flex align-items-baseline justify-content-between w-100 mt-3">
            <h5 v-if="getOrder.project_bid || (getOrder.on_hand_bid && getOrder.on_hand_bid.on_hand_bid_total)">Updated Total Balance: {{ getOrder.bidOrder && getOrder.bidOrder.order_finalTotal ? getOrder.bidOrder.order_finalTotal.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : getTotal }}</h5>
            <div v-if="bid_order_status_id == 7 && getOrder.refund && getOrder.bid_order_acc[0].bid_order_acc_type != 'Refund'" class="d-flex align-items-baseline">
                ...
                <div v-else class="d-flex align-items-baseline">
                    ...
                    <input type="text" class="form-control" style="width:150px" v-model="data.bid_order_acc_amount" :max="getOrder.bidOrder.order_dpAmount" onkeydown="return false" step=".5" min="0">
                </div>
            </div>
        </div>
        <div v-if="bid_order_status_id == 4" class="d-flex align-items-baseline justify-content-between w-100 mt-3">
            <h5 v-if="getOrder.bid_order_acc && getOrder.bid_order_acc.length > 0 && (getOrder.project_bid || getOrder.on_hand_bid)">Remaining Balance: {{ getInitialRemaining | toCurrency }}</h5>
        </div>
...
export default {
    ...
    created(){
        ...
        .then(() => {
            ...
            if(this.getOrder.bidOrder.order_negotiatedPrice){
                this.data.order_negotiatedPrice = this.getOrder.bidOrder.order_negotiatedPrice
            }
            else{
                this.data.order_negotiatedPrice = this.getOrder.bidOrder.order_initialPrice
            }
            ...
            if(this.getOrder.project_bid){
                total = (this.data.order_negotiatedPrice * this.getOrder.project_bid.project_bid_minQty) + ' - ' + (this.data.order_negotiatedPrice * this.getOrder.project_bid.project_bid_maxQty)
                totall = (this.getOrder.project_bid.project_bid_maxQty * this.data.order_negotiatedPrice)
            ...
            this.data.order_dpAmount = parseFloat(totall - (totall * 0.5))
            ...
            if(this.bid_order_status_id == 7 && this.getOrder.refund){
                if(this.getOrder.refund.refund_amount){
                    ...
                    this.data.bid_order_acc_amount = this.getOrder.bidOrder.order_dpAmount
                }
            }
            if((this.$route.query.finalPrice && this.$route.query.finalQty && this.$route.query.produce) || (this.bid_order_status_id == 4 && this.getOrder.on_hand_bid)){
                ...
                .then(() => {
                    ...
                    this.data.order_finalTotal = parseFloat(this.$route.query.finalPrice * this.$route.query.finalQty)
                    this.readyApp()
                })
                .catch((err) => {
                    console.log(err)
    ...
    methods:{
        ...
        validateAmount(e){
            var total = null;
            if(this.getOrder.project_bid){
                total = (this.getOrder.project_bid.project_bid_maxQty * this.data.order_negotiatedPrice)
            }
            else{
                total = (this.getOrder.on_hand_bid.on_hand_bid_qty * this.data.order_negotiatedPrice)
            }
            ...
            this.data.order_dpAmount = parseFloat(e.target.value)
        ...
        resetAmountPercentage(e){
            ...
            if(this.getOrder.project_bid){
                total = (this.getOrder.project_bid.project_bid_maxQty * this.data.order_negotiatedPrice)
                totall =(this.getOrder.project_bid.project_bid_minQty * this.data.order_negotiatedPrice)
        ...
        getTotall(){
            this.data.order_finalTotal = parseFloat(this.$route.query.finalPrice * this.$route.query.finalQty)
            return parseFloat(this.$route.query.finalPrice * this.$route.query.finalQty)
        },
        cancelTransaction(){
            var data = {
                id: this.getOrder.bidOrder.id,
    ...
    computed: {
        ...
        getInitialRemaining(){
            if(this.getOrder.project_bid){
                ...
                - parseFloat(this.getOrder.bid_order_acc[0].bid_order_acc_amount)).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
            }
        ...
        getTotal(){
            var total = null;
            if(this.getOrder.project_bid){
                total = (this.data.order_negotiatedPrice * this.getOrder.project_bid.project_bid_maxQty).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
                return total
            }
            else{
                total = (this.data.order_negotiatedPrice * this.getOrder.on_hand_bid.on_hand_bid_qty).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
                return total
            }
        },
        getInitialPayment(){
            ...
            return (total - (total * 0.5)).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
        },
        getPercentage() {
            var total = null;
            if(this.getOrder.project_bid){
        ...
        getRemaining(){
            var remaining
            if(this.bid_order_status_id == 4 && this.getOrder.on_hand_bid){
                remaining = parseFloat(this.getOrder.on_hand_bid.on_hand_bid_total - this.getOrder.bidOrder.order_dpAmount).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
                return remaining >= 0 ? remaining : parseFloat(0).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
            }
            else{
                remaining = parseFloat(this.getTotall() - parseFloat(this.getDeliveryForm.bid_order.order_dpAmount).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }))
                return remaining >= 0 ? remaining.toLocaleString("en-PH", { style: 'currency', currency: 'PHP' }) : parseFloat(0).toLocaleString("en-PH", { style: 'currency', currency: 'PHP' })
            }
        },
